...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	//ウィンドウのサイズを修正
	RECT rc1, rc2;
	GetWindowRect(hWnd, &rc1);  //ウィンドウの領域を取得
	...
	//FPS計算用変数
	int fpsCounter = 0;
	long long oldTick = GetTickCount64();  //前回の計測時の時間
	long long nowTick = oldTick;           //今回の計測時の時間

	//FPS固定用変数
	LARGE_INTEGER liWork;  //workがつく変数は作業用変数
	long long frequency;   //どれくらい細かく時間を設定できるか
	QueryPerformanceFrequency(&liWork);
	frequency = liWork.QuadPart;

	//時間（単位：カウント）取得
	QueryPerformanceCounter(&liWork);
	long long oldCount = liWork.QuadPart;  //前回の計測時の時間
	long long nowCount = oldCount;         //今回の計測時の時間
